...
BASE_RESULT_DIR="${BASE_RESULT_DIR:-experiments/simd_item2_intrinsics_only/sweep_results_avx512}"
...
run_case() {
  ...
  mkdir -p "${case_dir}"
  local out_json="${case_dir}/intrinsics_avx512_${INDEX_METHOD}.json"
  local out_csv="${case_dir}/intrinsics_avx512_${INDEX_METHOD}.csv"

  echo "==> Running intrinsics_avx512 (d=${d}, m=${m}, dsub=${dsub})"
...
if [[ "${AGGREGATE_ONLY}" == "1" ]]; then
  ...
  PYTHON_PATH="$(cat "${PYTHONPATH_FILE}")"
...
base = Path(os.environ.get("BASE_RESULT_DIR", "experiments/simd_item2_intrinsics_only/sweep_results_avx512"))
index_method = os.environ.get("INDEX_METHOD", "ivfpq")

rows = []
for case_dir in sorted(base.glob(f"d*_m*_dsub*_idx{index_method}")):
    metric_json = case_dir / f"intrinsics_avx512_{index_method}.json"
